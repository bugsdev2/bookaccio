...
import { processInterjections } from './processInterjections';

export function readingMotivation(startDate: number, endDate: number) {
  const duration = processDuration(startDate, endDate);
  const phrases = [
